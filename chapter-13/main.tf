# Lab 13.1 - Terraform for MongoDB Atlas
# Requires: Terraform >= 1.5, Atlas API keys in environment variables
...
# --- Atlas Project ---
resource "mongodbatlas_project" "nosql_labs" {
  name   = "NoSQL Labs - Training"
  org_id = var.atlas_org_id
}

# --- Free Tier Cluster ---
resource "mongodbatlas_cluster" "training" {
...
# --- Database User ---
resource "mongodbatlas_database_user" "lab_user" {
  project_id         = mongodbatlas_project.nosql_labs.id
  username           = "lab_user"
...
# --- IP Access (allow all for training) ---
resource "mongodbatlas_ip_access_list" "allow_all" {
  project_id = mongodbatlas_project.nosql_labs.id
